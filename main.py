...
def getfd():
    f = open_a_console("/proc/self/fd/0") or open_a_console("/dev/tty") or open_a_console(
        "/dev/tty0") or open_a_console("/dev/vc/0") or open_a_console("/dev/console")
    if f:
        return f
...
class ShowKey:
    def __init__(self):
        ...
        if self.fd == None:
            print
            "ERROR: Could not find appropriate file for monitoring. You might want to try 'sudo'"
            sys.exit(1)
        ...
        for keycomb, action in self.key_actions:
            if type(keycomb) == str:
                if pressed and keycomb == "*p":
                    Thread(target=action, args=(action, (kc,))).start()

                if not pressed and keycomb == "*r":
                    Thread(target=action, args=(kc,)).start()
                continue
            ...
            if result:
                try:
                    Thread(target=action, args=(None,)).start()
                except Exception as e:
                    print(e)

    def run(self):
        while True:
            buf: list = [some_char for some_char in os.read(self.fd, 1)]
            # print(buf)
            # print(len(buf))
            # print(buf[0])
            i_c = 0
            while i_c < len(list(buf)):
                c = buf[i_c]
                # print(c)
...
def key_pressed(kc, some_var):
    ...
    if some_var[0] == 28 or some_var[0] == 96:
        # enter
        pyautogui.click()
    return print("Key pressed - keycode: " + str(some_var))
